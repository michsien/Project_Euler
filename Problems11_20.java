...
// Problem 11 : Find the greatest product of N adjacent numbers in the same 
// direction (up, down, left, right, or diagonally) in the SIZE×SIZE grid
  ...
  public static void problem11() {
    ...
    for (int i = 0; i < SIZE; i++) {
      for (int j = 0; j < SIZE; j++) {
        //vertically and diagonally (downwards)
        if (i + N <= SIZE) {
          int vertical = grid[i][j];
          for (int k = i + 1; k < i + N; k++) {
            vertical *= grid[k][j];
          }
          maximum = Math.max(maximum, vertical);
          if (j + N <= SIZE) {
            int diagonalDown = grid[i][j];
            for (int k = 1; k < N; k++) {
              diagonalDown *= grid[i + k][j + k];
            }
            maximum = Math.max(maximum, diagonalDown);
          }
        }
        //horizontally and diagonally (upwards)
        if (j + N <= SIZE) {
          int horizontal = grid[i][j];
          for (int k = j + 1; k < j + N; k++) {
            horizontal *= grid[i][k];
          }
          maximum = Math.max(maximum, horizontal);
          if (i - N >= -1) {
            int diagonalUp = grid[i][j];
            for (int k = 1; k < N; k++) {
              diagonalUp *= grid[i - k][j + k];
            }
            maximum = Math.max(maximum, diagonalUp);
          }
        }
      }
    }
